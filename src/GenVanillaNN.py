...
class GenNNSkeImToImage(nn.Module):
    def __init__(self):
        ...
        # Input: 3 x 256 x 256
        self.enc1 = nn.Sequential(nn.Conv2d(3, 64, 4, 2, 1, bias=False), nn.LeakyReLU(0.2, inplace=True))       # -> 128
        self.enc2 = nn.Sequential(nn.Conv2d(64, 128, 4, 2, 1, bias=False), nn.BatchNorm2d(128), nn.LeakyReLU(0.2, inplace=True)) # -> 64
        self.enc3 = nn.Sequential(nn.Conv2d(128, 256, 4, 2, 1, bias=False), nn.BatchNorm2d(256), nn.LeakyReLU(0.2, inplace=True)) # -> 32
        self.enc4 = nn.Sequential(nn.Conv2d(256, 512, 4, 2, 1, bias=False), nn.BatchNorm2d(512), nn.LeakyReLU(0.2, inplace=True)) # -> 16
...
class GenVanillaNN():
    ...
    def generate(self, ske):
        ...
        with torch.no_grad(): normalized_output = self.netG(ske_t_batch)
