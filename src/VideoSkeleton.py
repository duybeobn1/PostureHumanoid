...
class VideoSkeleton:
    ...
    def __init__(
        ...
        filename_pkl, filename_dir, filename_base = filename_change_ext(
            filename, ".pkl"
        )
        ...
        if (
            os.path.exists(filename_pkl)
            and os.path.exists(filename_dir)
            and not forceCompute
        ):
        ...
        for i in range(video.getTotalFrames()):
            ...
            if widthIsSet == False:
                ...
                print(
                    "new_video_width="
                    + str(self.new_video_width)
                    + " new_video_height="
                    + str(self.new_video_height)
                )
                print(
                    " ske_width_crop="
                    + str(self.ske_width_crop)
                    + " ske_height_crop="
                    + str(self.ske_height_crop)
                )
            ...
            if i % self.mod_frame == 0:
                ...
                if isSke:
                    ...
                    print(
                        ...
                        + str(i)
                        + "/"
                        + str(video.getTotalFrames())
                        + "   filename="
                        + filename_im
                        + "  save="
                        + filename_imsave
                        + " sizeof="
                        + str(sys.getsizeof(self.ske))
                    )
    ...
    def setWidthHeigh(self, image, newVideoWidth, cropRatio):
        if newVideoWidth == -1:
            ...
            self.new_video_height = int(
                image.shape[0] * self.new_video_width / image.shape[1]
            )
        self.ske_width_crop = int(
            cropRatio * self.new_video_width
        )  # skeleton image is resized to this width
        self.ske_height_crop = int(
            cropRatio * self.new_video_height
        )  # skeleton image is resized to this height
    ...
    def cropAndSke(self, image, ske, isCrop=True):
        ...
        if ske.fromImage(image):  # a skeleton is found
            ...
            if isCrop:
                ...
                image = crop_with_padding(
                    image, xm, ym, self.ske_width_crop, self.ske_height_crop
                )
                # print("crop: xm="+str(xm) + " xM="+str(xM) + " ym="+str(ym) + " yM="+str(yM))
                ske.crop(
                    xm / self.new_video_width,
                    ym / self.new_video_height,
                    self.ske_width_crop / self.new_video_width,
                    self.ske_height_crop / self.new_video_height,
                )  # skeleton crop
    ...
    def load(cls, filename):
        ...
        print(
            ...
            + filename
            + " #skeleton="
            + str(objet_charge.ske.shape)
            + " #image="
            + str(objet_charge.im.shape)
        )
        return objet_charge
    ...
    def draw(self):
        ...
        for i in range(self.skeCount()):
            ...
            empty = np.zeros(
                (self.ske_height_crop, self.ske_width_crop, 3), dtype=np.uint8
            )
            im = self.readImage(i)
